...
IntCodeMachine::IntCodeMachine(const IntCodeMachine& from) :
    state(from.state),
    programPointer(from.programPointer),
    relativeBase(from.relativeBase),
    program(from.program),
    inputQueue(from.inputQueue),
    outputQueue(from.outputQueue),
    outputHistory(from.outputHistory)
{}
...
void IntCodeMachine::queueInput(const string& inputString) {
    for (char c : inputString) {
        inputQueue.emplace((long long)c);
    }
}
...
string IntCodeMachine::readOutputString() {
    string ret(outputQueue.size(), '\0');

    int index = 0;
    while (!outputQueue.empty()) {
        ret[index++] = (char)outputQueue.front();
        outputQueue.pop();
    }
